  ...
  def add_to_cart
    ...
    if painting
      ...
      if last_created_line_item.present?
        ...
        last_created_line_item.update(amount: (last_created_line_item.painting.price.to_i + last_created_line_item.option.price.to_i ) * params["line_item"]["quantity"].to_i + past_amount, quantity: past_quantity + params["line_item"]["quantity"].to_i)
        @flag = true
      else
        # 신규 등록일 경우, LineItem 을 추가한다.
        if cart.line_items.create!(line_item_params)
          (last_created_line_item = cart.line_items.first).update amount: (last_created_line_item.painting.price.to_i + last_created_line_item.option.price.to_i ) * params["line_item"]["quantity"].to_i
          @flag = true
        end
      end
    end
